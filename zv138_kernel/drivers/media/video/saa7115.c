...
const unsigned char reg_saa7121h_config[45][2]={
...
/*
const unsigned char reg_saa7121h_config[96][2] = {
	// NULL register
		{0x00, 0x00},
		{0x01, 0x00},
		{0x02, 0x00},
		...
		{0x3a, 0x13},  //  1 // modified at 2005.10.6    // bt656 sync	
		//	{0x3a, 0x03},  //  1  // RCV receive sync signal
		...
		{0x69, 0x00},
		{0x6a, 0x00},
		{0x6b, 0x00},         // modified at 2005.10.6    // bt656 sync
		//	{0x6b, 0x20},         // RCV receive sync signal
		{0x6c, 0x02},
		{0x6d, 0x20},
		{0x6e, 0x80},
		{0x6f, 0x14},
...
static inline int saa711x_write(struct v4l2_subdev *sd, u8 reg, u8 value)
{
	...
	printk(KERN_WARNING"Writing %d to R%d!!!!!!!!!\n",value,reg);
	ret = i2c_smbus_write_byte_data(client, reg, value);
	if(ret)
		printk(KERN_WARNING"Writing %d to R%d!!!!!!!!ret=%d faild!!\n",value,reg,ret);
	else
		printk(KERN_WARNING"Writing %d to R%d!!!!!!!!! success!!\n",value,reg);
	return ret;
...
static inline int saa711x_read(struct v4l2_subdev *sd, u8 reg)
{
	...
	printk(KERN_WARNING"Reading %d from R%d!!!!!!!!!!\n",value,reg);
	 return value;
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA))
		return -EIO;
	printk(KERN_WARNING"SAA7121 PROBE!!!!!!!!!!!!!!!!!!\n");
/*	for (i = 0; i < 0x0f; i++) {
...
/*	state->crystal_freq = SAA7115_FREQ_24_576_MHZ;
	...
	for(i=0;i<45;i++)
	{
		saa711x_write(sd,reg_saa7121h_config[i][0],reg_saa7121h_config[i][1]);
	}
	return 0;
}
