...
#define DA850_MMCSD_CD_PIN		GPIO_TO_PIN(6, 9)
#define DA850_MMCSD_WP_PIN		GPIO_TO_PIN(6, 10)
...
#define GPIO2_2_PIN				GPIO_TO_PIN(2,2)
...
static u8 da850_iis_serializer_direction[] = {
	...
	INACTIVE_MODE,	INACTIVE_MODE,	INACTIVE_MODE,	RX_MODE,
	INACTIVE_MODE,	INACTIVE_MODE,	INACTIVE_MODE,	INACTIVE_MODE,
...
static __init void da850_evm_init(void)
{
	...
	if (!HAS_GLCD && HAS_VPIF_DISPLAY) {
		...
		if (ret)
			pr_warning("da850_evm_init: vpif capture mux failed: "
					"%d\n",	ret);
		gpio_request(GPIO2_2_PIN, "vpif_ctl");
		gpio_direction_output(GPIO2_2_PIN,0);
	...
	/*********led function************/
	ret= da8xx_pinmux_setup(da850_led_pins);
	if(ret)
            pr_warning("da850_evm_init: led mux failed: "
                    "%d\n", ret);
